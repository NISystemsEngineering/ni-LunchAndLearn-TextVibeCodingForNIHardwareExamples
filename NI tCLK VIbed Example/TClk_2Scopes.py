...
Triggers on a rising edge and synchronizes acquisition across two scopes.
"""
...
class App(tk.Tk):
    def __init__(self):
        ...
        if not NISCOPE_AVAILABLE:
            self._status(
                "niscope / nitclk not found — running in simulation mode", WARN)
    ...
    def _build_resources_frame(self, parent):
        ...
        lf.grid(row=0, column=0, columnspan=4, sticky="ew",
                padx=(0, 0), pady=(0, 12))
    ...
    def _build_section(self, parent, title, row, col, populate_fn):
        lf = ttk.LabelFrame(parent, text=title, padding=14)
        lf.grid(row=row, column=col, sticky="nsew",
                padx=(0, 8) if col < 3 else (0, 0), pady=(0, 12))
    ...
    def _build_buttons(self, parent):
        ...
        frame.grid(row=2, column=0, columnspan=4, sticky="w", pady=(4, 0))
        ...
        self.run_btn = ttk.Button(
            frame, text="Run Measurement", command=self._on_run,
    ...
    def _build_results_tab(self):
        self.res_tab.rowconfigure(0, weight=3)
        ...
        # ── Measurements table ─────────────────────────────────────────────────
        meas_lf = ttk.LabelFrame(
            self.res_tab, text="  Measurements  ", padding=8)
        meas_lf.grid(row=1, column=0, sticky="nsew", padx=12, pady=(0, 12))
        meas_lf.columnconfigure(0, weight=1)
        meas_lf.rowconfigure(0, weight=1)

        cols = ("Scope", "Resource", "Channel",
                "Vpp (V)", "Vmax (V)", "Vmin (V)",
                "Vrms (V)", "Mean (V)", "Std Dev (V)", "Samples")
        self.tree = ttk.Treeview(
            meas_lf, columns=cols, show="headings", height=4)
        for col in cols:
            self.tree.heading(col, text=col)
            w = 110 if "(V)" in col else 80
            self.tree.column(col, width=w, anchor="center", minwidth=60)

        vsb = ttk.Scrollbar(meas_lf, orient="vertical",
                             command=self.tree.yview)
        self.tree.configure(yscrollcommand=vsb.set)
        self.tree.grid(row=0, column=0, sticky="nsew")
        vsb.grid(row=0, column=1, sticky="ns")

        self._tdiff_lbl = tk.Label(
            meas_lf, text="ΔT at 0.1 V (rising):   —",
            fg=DIM, bg=BG, font=("Segoe UI", 10), anchor="w")
        self._tdiff_lbl.grid(row=1, column=0, columnspan=2,
                              sticky="w", pady=(6, 0))
        ...
        def _worker():
            try:
                ...
                for res in resources:
                    ...
                    sessions.append(s)
                self._post(lambda s=sessions: self._on_connected(s, resources))
            except Exception as exc:
                self._post(lambda e=exc: self._on_connect_error(e))

        threading.Thread(target=_worker, daemon=True).start()

    def _on_connected(self, sessions, resources):
        self.sessions = sessions
        self._status(f"Connected: {', '.join(resources)}", SUCCESS)
        self._conn_lbl.config(text=f"Connected ({NUM_SCOPES} scopes)", fg=SUCCESS)
        self.connect_btn.config(state=tk.DISABLED)
    ...
    def _on_run(self):
        ...
        self._status("Starting measurement…", WARN)
        threading.Thread(target=self._measure_thread, daemon=True).start()

    def _on_stop(self):
        self.running = False
        self.stop_btn.config(state=tk.DISABLED)
        self.run_btn.config(state=tk.NORMAL if self.sessions else tk.DISABLED)
    ...
    def _measure_thread(self):
        try:
            ...
            self._post(lambda: self._status("Synchronizing with TClk…", WARN))
            self._tclk_initiate(cfg)

            if not self.running:
                return

            self._post(lambda: self._status("Fetching waveforms…", WARN))
            results = self._fetch_all(cfg)

            self._post(lambda r=results: self._update_display(r))
            self._post(lambda: self._status("Measurement complete", SUCCESS))
            ...
            self._post(lambda: self.run_btn.config(
                state=tk.NORMAL if self.sessions else tk.DISABLED))
            self.running = False
    ...
    def _update_display(self, results):
        ...
        for i, r in enumerate(results):
            ...
            vmax = float(np.max(y))
            vmin = float(np.min(y))
            vrms = float(np.sqrt(np.mean(y ** 2)))
            mean = float(np.mean(y))
            std  = float(np.std(y))

            self.tree.insert("", "end", tags=(f"scope{i}",), values=(
                f"Scope {r['scope']}", r["resource"], r["channel"],
                f"{vpp:.5f}", f"{vmax:.5f}", f"{vmin:.5f}",
                f"{vrms:.5f}", f"{mean:.5f}", f"{std:.5f}",
                len(y)))

        self.ax.relim()
        self.ax.autoscale_view()

        # Time difference between channels at 0.1 V rising edge
        threshold = 0.1
        crossings = [
        ...
        if all(c is not None for c in crossings):
            ...
            self._tdiff_lbl.config(
                text=f"ΔT at {threshold} V (rising):   {dt * 1e9:.3f} ns"
                     f"   ({results[0]['resource']} → {results[1]['resource']})",
                fg=ACCENT)
        else:
            missing = [results[i]["resource"]
                       for i, c in enumerate(crossings) if c is None]
            self._tdiff_lbl.config(
                text=f"ΔT at {threshold} V (rising):   no crossing found"
                     f" on {', '.join(missing)}",
                fg=WARN)
        ...
        for i in range(NUM_SCOPES):
            self.tree.tag_configure(
                f"scope{i}", foreground=SCOPE_COLORS[i])
