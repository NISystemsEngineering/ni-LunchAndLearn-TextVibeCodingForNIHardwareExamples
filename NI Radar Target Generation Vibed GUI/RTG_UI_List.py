"""PyQt6 desktop UI for controlling NI RTG using Target Configuration List (CSV) files."""
import sys
...
class RtgWorker(QObject):
    ...
    def send_list_file(self, device: int, file_path: str) -> None:
        try:
            self._client.send_target_list_files(device, [file_path])
            self.result.emit(f"Sent list file: {file_path}")
        except RtgError as e:
            self.error.emit(str(e))
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("NI Radar Target Generation Control - List Files")
    ...
    def _build_ui(self) -> None:
        ...
        list_layout.addWidget(self._build_list_files_group())
        list_layout.addStretch()
        self.tabs.addTab(list_tab, "List Files")
    ...
    def _build_list_files_group(self) -> QGroupBox:
        box = QGroupBox("Target Configuration List File")
        layout = QVBoxLayout(box)
        layout.setSpacing(6)
        ...
        self.list_file_edit.setPlaceholderText("Select a CSV file...")
        self.list_file_edit.setMinimumWidth(350)
        file_row.addWidget(self.list_file_edit)
        self.btn_browse = QPushButton("Browse...")
        self.btn_browse.clicked.connect(self._on_browse_csv)
        file_row.addWidget(self.btn_browse)
        layout.addLayout(file_row)

        btn_row = QHBoxLayout()
        self.btn_send_list = QPushButton("Send File")
        self.btn_send_list.clicked.connect(self._on_send_list_file)
        self._style_btn(self.btn_send_list, "#1565c0")
        btn_row.addWidget(self.btn_send_list)
        btn_row.addStretch()
    ...
    def _set_connected(self, connected: bool) -> None:
        ...
        self.btn_send_list.setEnabled(connected)
    ...
    def _auto_start(self) -> None:
        ...
        if state in ("idle", "inactive"):
            self._worker.start()
            self.tabs.setCurrentIndex(2)  # switch to List Files tab
    ...
    DEFAULT_LIST_DIR = r"C:\Users\bruce wayne\Desktop\RTG List Files"

    def _on_browse_csv(self) -> None:
        start_dir = self.list_file_edit.text().strip() or self.DEFAULT_LIST_DIR
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Target Configuration CSV", start_dir, "CSV Files (*.csv);;All Files (*)"
        )
        if path:
            self.list_file_edit.setText(path)

    def _on_send_list_file(self) -> None:
        path = self.list_file_edit.text().strip()
        if not path:
            self._log_error("No CSV file selected.")
            return
        windows_path = path.replace("/", "\\")
        self._worker.send_list_file(self.device_spin.value(), windows_path)
        self._worker.send_software_trigger(self.device_spin.value())
